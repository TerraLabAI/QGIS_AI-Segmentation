"""CloudPredictor - same interface as SamPredictor but calls a remote SAM2 server."""
import json
import base64
...
CLOUD_URL = "https://sam-api.orangetree-f87986eb.westeurope.azurecontainerapps.io"
_TIMEOUT_HEALTH = 60
...
class CloudPredictor:
    ...
    def _request(self, method, path, data=None, timeout=30):
        ...
        try:
            ...
            try:
                ...
                pass
            raise RuntimeError(
                "Cloud server error {}: {}".format(e.code, detail)
            )
        except urllib.error.URLError as e:
            raise RuntimeError(
                "Cloud server unreachable: {}".format(e.reason)
            )
        ...
        try:
            ...
            if resp.get("status") == "ok":
                QgsMessageLog.logMessage(
                    "Cloud server connected (device: {})".format(
    ...
    def set_image(self, image_np: np.ndarray) -> None:
        ...
        QgsMessageLog.logMessage(
            "Cloud set_image: session={}, size={}".format(
    ...
    def predict(
        ...
        resp = self._request("POST", "/predict", data, timeout=_TIMEOUT_PREDICT)
